...
버튼의 범위에서 클릭이 들어오면 색상 테마를 포함한 모든 설정값들을 초기화하고 해당하는 프랙탈 또는 테마(문자열로 적혀있다)를 실행하고, 박스의 범위에서 클릭이 들어오면 현재 어떤 프랙탈이 켜져 있는지 탐색해서 해당하는 기능을 수행한다.
...
휠을 클릭하면 2D 프랙탈의 색상 테마를 제외한 모든 설정값들을 초기화한다. 확대, 평행이동 등을 기본값으로 돌려놓는다. 처음에 버튼을 켰을 때의 상태로 돌아가는 걸 볼 수 있다.
